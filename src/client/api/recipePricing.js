...
import ingredientPricingMasterJSON from '../json/ingredientPricingMaster.json';
	...
	getPrice(item) {
		...
		ingredients.forEach((ingredient) => {
			if (ingredient.item) {
				...
				const profit = 1;
				let totalPriceForRequiredQty = (ingredientPricingMasterJSON[ingredient.item] * qty) / 10;
				totalPriceForRequiredQty = totalPriceForRequiredQty + (totalPriceForRequiredQty * profit);
				price += totalPriceForRequiredQty;
				console.log('--Price of '+ingredient.item+'--: ', ingredientPricingMasterJSON[ingredient.item]);
			}
		})
		return price;
	}
